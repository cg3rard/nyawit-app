    ...
    if llm_pipeline is None:
        try:
            ...
            if status == "Merah":
                return {
                    "action": "RESTOCK_URGENT",
                    "recommendation": f"Terbitkan purchase order (PO) darurat ke supplier utama hari ini. (Fallback: {e})",
                    "rationale": "Sisa stok kritis berada di bawah batas aman operasional dengan permintaan aktif."
                }
            elif status == "Kuning":
                return {
                    "action": "PROMO_DISKON",
                    "recommendation": f"Terapkan diskon bundling 20% atau relokasi barang ke rak display kasir. (Fallback: {e})",
                    "rationale": "Terjadi penurunan penjualan signifikan dalam 7 hari terakhir dengan persediaan menumpuk."
                }
            else:
                return {
                    "action": "PERTAHANKAN_STOK",
                    "recommendation": f"Pertahankan siklus replenishment reguler sesuai jadwal mingguan. (Fallback: {e})",
                    "rationale": "Tingkat perputaran barang dan persediaan berada pada batas optimal."
                }
...
def get_available_scenarios():
    ...
    if not os.path.exists(MOCK_DATA_PATH):
        raise HTTPException(status_code=404, detail=f"File mock_data.json tidak ditemukan di: {MOCK_DATA_PATH}")
